...
| 7 | `completed` | 完了状態 | `BOOLEAN` | 不可 | `FALSE` | `FALSE`=未完了、`TRUE`=完了。MySQLでは内部的に `0` / `1` として扱われることがある。画面には保存値を直接出さず、未完了 / 完了と表示する |
...
| 完了にチェックを入れる | `completed` | 未完了=`FALSE`、完了=`TRUE`を保存。MySQLでは内部的に `0` / `1` として扱われることがある |
| 一覧にやること・ジャンル・優先度・期限・状態を表示する | `title`、`category`、`priority`、`due_date`、`completed` | `priority` と `completed` は画面表示用の言葉に変換し、`completed` は未完了 / 完了と表示する |
